...
function decompress(input: Uint8Array, originalSize: number): Uint8Array {
  const inputBuffer = input;
  const outputBuffer = new Uint8Array(originalSize);

  const inputPtr = zstdModule._malloc(inputBuffer.length);
  const outputPtr = zstdModule._malloc(originalSize);

  zstdModule.HEAPU8.set(inputBuffer, inputPtr);
